import { Table } from "antd";
import { Link } from "react-router-dom";
import { CustomHeader } from "../../components/Shared/common/CustomHeader";
import { Hospital_RB } from "../../components/Shared/Modals/Hospital_RB";
import { ROUTES } from "../../route/constant";

export const Health_Service = () => {
  const columns = [
    {
      title: "S.No",
      dataIndex: "sno",
      sorter: (a, b) => a.sno - b.sno,
      sortDirections: ["descend"],
    },
    {
      title: " Hospital Name",
      dataIndex: "name",
      sorter: (a, b) => a.name.length - b.name.length,
      sortDirections: ["descend"],
    },
    {
      title: "Type",
      dataIndex: "type",
    },
    {
      title: "Contact Number",
      dataIndex: "contact",
    },
    {
      title: "Description",
      dataIndex: "description",
    },

    {
      title: "Status",
      dataIndex: "status",
    },
    {
      title: "Action",
      dataIndex: "action",
    },
  ];
  const data = [
    {
      sno: 1,
      name: "Apollo",
      contact: "9809898909",
      type: "private",
      description: "Top Private Hospital",
      status: (
        <div>
          <input
            type="checkbox"
            id="switch01"
            defaultChecked="true"
            data-switch="success"
          />
          <label
            htmlFor="switch01"
            data-on-label="Active"
            data-off-label="In-active"
            className="mb-0 d-block"
            style={{ width: "70px" }}
          />
        </div>
      ),
      action: (
        <div>
          <a href="#" className="action-icon">
            <i className="mdi mdi-pencil font-20 icon-color" />
          </a>
          <a href="#" className="action-icon">
            <i className="mdi mdi-delete font-20 icon-color " />
          </a>
          <a href="#" className="action-icon">
            <i className="uil uil-eye font-20 icon-color" />
          </a>

          <Link
            href="#"
            className="action-icon"
            style={{ fontSize: "20px", marginLeft: "5px" }}
            to={ROUTES.AMBULANCE_LIST}
          >
            <i className=" mdi mdi-ambulance icon-color" />
          </Link>
        </div>
      ),
    },
  ];
  const onChange = (pagination, filters, sorter, extra) => {
    console.log("params", pagination, filters, sorter, extra);
  };
  return (
    <>
      <div>
        <CustomHeader
          title={"Health Services"}
          title1={"Health Services"}
          title2={"Hospitals"}
        />

<div className="row ">
          <div className="col-lg-12">
            <div className="card p-2">
              <div className="row g-0 align-items-center">
                <div className="col-3">
                  <div className="input-group">
                    <input
                      type="search"
                      className="form-control dropdown-toggle "
                      placeholder="Search..."
                      id="top-search"
                    />
                  </div>
                </div>
                <div className="col-9">
                  <div className="row">
                    <div className="col-3"></div>
                    <div className="col-3 d-flex flex-column justify-content-end">
                      
                      {/* <select class="form-select" id="example-select">
                      <option selected>Filter Info</option>
                        <option>Water Information</option>
                        <option>Power Information</option>
                        <option>Garbage Information</option>
                      </select> */}
                    </div>
                    <div className="col-3 d-flex flex-column justify-content-end">
                      
                      <select class="form-select" id="example-select">
                      <option selected>Filter Hospital</option>
                        <option>Bhandari Hospital</option>
                        <option>CHL Hospital</option>
                        <option>Apollo Hospital</option>
                        <option>Jyoti Hospital</option>
                      </select>
                    </div>

                    <div className="col-3 d-flex justify-content-end">
                    <button
                    type="button"
                    class="btn btn-primary w-100"
                    style={{ border: "none" }}
                    data-bs-toggle="modal"
                    data-bs-target="#Hospital_modal"
                  >
                    Add Hospital
                  </button>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>

        {/* <div className="row ">
          <div className="col-lg-12">
            <div className="card p-2">
              <div className="row g-0 align-items-center">
                <div style={{ display: "flex", justifyContent: "end" }}>
                  <button
                    type="button"
                    class="btn btn-primary "
                    style={{ border: "none" }}
                    data-bs-toggle="modal"
                    data-bs-target="#Hospital_modal"
                  >
                    Add Hospital
                  </button>
                </div>
              </div>
            </div>
          </div>
        </div> */}
        <div className="row ">
          <div className="col-lg-12">
            <div className="card p-2">
              <div className="row g-0 align-items-center">
                <div>
                  <Table
                    columns={columns}
                    dataSource={data}
                    onChange={onChange}
                  />
                </div>{" "}
              </div>{" "}
            </div>
          </div>{" "}
        </div>

        <Hospital_RB />
      </div>
    </>
  );
};
